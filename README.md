*This project has been created as part of the 42 curriculum by elarue and wakhazza.*
...
* `--simple` : force l’algorithme à utiliser le **tri par selection**.
...
1. **Simple** : basé sur le **tri par sélection**. Efficace pour les petites structures ; il minimise les échanges en cherchant systématiquement le plus petit élément pour le placer à sa position définitive.
...
* **Désordre < 0.2 (faible)** : utilise l’**selection sort** pour viser une complexité proche de l’optimal sur des listes presque triées.
...
* **wakhazza** :
  ...
  * implémentation des algorithmes **Simple** (selection sort) et **Medium** (chunk sort) ;
...
* [Tri par selection — Wikipedia](https://fr.wikipedia.org/wiki/Tri_par_s%C3%A9lection)

### Utilisation de l’IA

L’IA a été utilisée comme assistant pédagogique pour la tâches suivante :
